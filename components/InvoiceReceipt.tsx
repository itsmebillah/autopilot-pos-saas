...
export default function InvoiceReceipt({
  ...
  return (
    <div className="invoice-receipt-wrapper select-text font-sans antialiased text-black">
      {/* Dynamic Print CSS */}
      <style jsx global>{`
        @media print {
          @page {
            margin: ${template === "a4_standard" ? "12mm" : "0mm"};
          ...
          /* Hide all UI layout elements outside print preview */
          ...
          .invoice-receipt-container {
            ...
            padding: ${template === "thermal_58mm" ? "4px" : template === "thermal_80mm" ? "8px" : "0px"} !important;
            width: 100% !important;
...
/* ==========================================================================
   1. THERMAL 58MM COMPACT LAYOUT (Width ~58mm / 210px)
   ========================================================================== */
function Thermal58mmLayout({
  ...
  return (
    <div className="invoice-receipt-container bg-white text-black p-3 mx-auto w-full max-w-[220px] text-[11px] leading-tight font-mono">
      {/* Reprint Banner */}
      {isReprint && (
        <div className="text-center font-bold uppercase tracking-widest text-[9px] border-y border-black py-0.5 mb-1.5">
          *** DUPLICATE REPRINT ***
        </div>
      )}
      ...
      <div className="text-center pb-2 border-b border-dashed border-black">
        {business.logo_url && (
          <img src={business.logo_url} alt="Logo" className="w-10 h-10 object-contain mx-auto mb-1" />
        )}
        <h1 className="font-bold text-xs uppercase tracking-tight">{business.name}</h1>
        {business.address && <p className="text-[9px] text-gray-700 mt-0.5">{business.address}</p>}
      ...
      <div className="py-1.5 border-b border-dashed border-black text-[9px] space-y-0.5">
        ...
        {customer?.name && (
          <div className="pt-0.5 border-t border-dotted border-gray-400">
            <span>Cust: {customer.name} {customer.phone ? `(${customer.phone})` : ""}</span>
          </div>
        )}
      </div>
      ...
      <div className="py-2 border-b border-dashed border-black">
        <div className="flex justify-between font-bold border-b border-black pb-1 mb-1 text-[9px]">
          <span>ITEM</span>
          <span>QTY x PR</span>
          <span>TOT</span>
        </div>
          ...
          {items.map((item: any, idx: number) => (
            <div key={idx} className="space-y-0.5">
              ...
              <div className="flex justify-between text-[10px]">
                ...
                <span className="font-bold">{formatCurrency(item.total, config)}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="py-2 border-b border-dashed border-black space-y-1 text-[10px]">
        ...
        {totals.discount_total > 0 && (
          <div className="flex justify-between text-gray-700">
            <span>Discount:</span>
            <span>-{formatCurrency(totals.discount_total, config)}</span>
          </div>
        )}
        {totals.tax_total > 0 && (
          <div className="flex justify-between text-gray-700">
      ...
      <div className="pt-2 text-center text-[9px] space-y-1">
        ...
        <p className="italic text-[8px]">{config.footer_message}</p>
        <p className="text-[7px] text-gray-500">Autopilot POS SaaS</p>
      </div>
    </div>
  );
}
...
function Thermal80mmLayout({
  ...
  return (
    <div className="invoice-receipt-container bg-white text-black p-4 sm:p-5 mx-auto w-full max-w-[320px] text-xs leading-normal font-sans">
      {/* Reprint Banner */}
      {isReprint && (
        <div className="bg-black text-white text-center font-bold uppercase tracking-widest text-[10px] py-1 rounded mb-3">
          *** DUPLICATE REPRINT ***
        </div>
      )}
      ...
      <div className="text-center pb-3 border-b-2 border-dashed border-gray-400">
        ...
        <h1 className="font-bold text-base uppercase tracking-tight text-gray-900">{business.name}</h1>
        {business.store_name && (
          <p className="text-xs font-semibold text-gray-700">{business.store_name}</p>
        )}
        {business.address && (
          <p className="text-[11px] text-gray-600 mt-0.5">{business.address}</p>
        )}
        {business.phone && (
      ...
      <div className="py-2.5 border-b border-dashed border-gray-300 text-[11px] space-y-1">
        ...
        {customer && (
          <div className="pt-1.5 border-t border-dotted border-gray-300 mt-1">
      ...
      <div className="py-3 border-b-2 border-dashed border-gray-400">
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-gray-900 font-bold text-[11px]">
            ...
            {items.map((item: any, idx: number) => (
              <tr key={idx} className="align-top">
                <td className="py-1.5 pr-1">
                  ...
                  {item.sku && <div className="text-[10px] text-gray-500 font-mono">SKU: {item.sku}</div>}
                  {item.serial_numbers?.length > 0 && (
                    <div className="text-[10px] text-gray-600">SN: {item.serial_numbers.join(", ")}</div>
                  )}
                  {item.batch_number && (
                    <div className="text-[10px] text-gray-500">Batch: {item.batch_number}</div>
                  )}
                </td>
      ...
      <div className="py-3 border-b border-dashed border-gray-400 space-y-1.5 text-xs">
        ...
        {totals.discount_total > 0 && (
          <div className="flex justify-between text-gray-700">
            ...
            <span className="font-mono text-green-700">-{formatCurrency(totals.discount_total, config)}</span>
          </div>
        )}
        {totals.tax_total > 0 && (
          <div className="flex justify-between text-gray-600">
            <span>{config.tax_label || "VAT"}:</span>
            <span className="font-mono">{formatCurrency(totals.tax_total, config)}</span>
          </div>
        )}
        <div className="flex justify-between font-bold text-sm border-t-2 border-gray-900 pt-2 text-gray-900">
          <span>NET PAYABLE:</span>
          <span className="font-mono">{formatCurrency(totals.grand_total, config)}</span>
        </div>

        {/* Tendered Payments Breakdown */}
        <div className="pt-2 border-t border-dotted border-gray-300 space-y-1">
          {payments.map((p: any, idx: number) => (
            <div key={idx} className="flex justify-between text-[11px] text-gray-700">
              <span className="uppercase font-medium">Tender ({p.method}):</span>
              <span className="font-mono">{formatCurrency(p.amount, config)}</span>
            </div>
          ))}
          {totals.change_amount > 0 && (
            <div className="flex justify-between text-xs font-semibold text-gray-900 pt-1">
              <span>Change Returned:</span>
              <span className="font-mono">{formatCurrency(totals.change_amount, config)}</span>
            </div>
          )}
          {totals.due_amount > 0 && (
            <div className="flex justify-between text-xs font-bold text-red-600 pt-1">
              <span>Remaining Balance:</span>
              <span className="font-mono">{formatCurrency(totals.due_amount, config)}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="pt-3 text-center space-y-2">
        <div
          ...
          dangerouslySetInnerHTML={{ __html: barcodeSvg }}
        />
        <p className="font-mono text-[10px] text-gray-600 tracking-wider font-bold">
        ...
        {config.return_policy && (
          <div className="bg-gray-50 border border-gray-200 rounded p-2 text-[10px] text-gray-600 leading-tight text-left">
            <span className="font-bold block text-gray-800 mb-0.5">Policy Notice:</span>
            {config.return_policy}
          </div>
        )}

        <div className="pt-2 text-[9px] text-gray-400 font-mono">
          Powered by Autopilot POS Cloud Platform
        </div>
      </div>
    </div>
  );
...
/* ==========================================================================
   3. A4 PROFESSIONAL INVOICE LAYOUT (Standard A4 Corporate Sheet)
   ========================================================================== */
function A4StandardLayout({
  ...
  return (
    <div className="invoice-receipt-container bg-white text-black p-8 sm:p-12 mx-auto w-full max-w-4xl text-sm leading-normal font-sans shadow-lg print:shadow-none print:p-0">
      {/* Reprint Banner */}
      {isReprint && (
        <div className="bg-black text-white text-center font-bold uppercase tracking-widest text-xs py-1.5 rounded mb-6">
          *** OFFICIAL INVOICE COPY / DUPLICATE REPRINT ***
        </div>
      )}

      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row justify-between items-start gap-6 pb-8 border-b-2 border-gray-900">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 max-w-sm mt-1">{business.address}</p>
        ...
        <div className="text-left sm:text-right bg-gray-50 border border-gray-200 rounded-2xl p-4 min-w-[220px]">
          ...
          <span className="text-lg font-mono font-bold text-gray-950 block">
            {transaction.invoice_no}
          </span>
          <div className="mt-2 pt-2 border-t border-gray-200 text-xs space-y-1 text-gray-600">
            <div className="flex justify-between gap-4">
              <span className="font-medium">Date:</span>
              <span className="font-mono">{transaction.date_formatted}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="font-medium">Time:</span>
              <span className="font-mono">{transaction.time_formatted}</span>
            </div>
            <div className="flex justify-between gap-4">
      ...
      {/* Bill To & Reference Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 my-6 p-4 bg-gray-50/70 border border-gray-200 rounded-xl text-xs">
        <div>
          <h3 className="font-bold text-gray-900 uppercase tracking-wider text-[11px] mb-1.5">
            Billed To:
          </h3>
          {customer ? (
        ...
        <div className="sm:border-l sm:border-gray-200 sm:pl-6">
          ...
          <div className="space-y-1 text-gray-700">
            ...
            <div className="flex justify-between">
              <span className="font-medium">Payment Tender:</span>
            ...
            {items.map((item: any, idx: number) => (
              <tr key={idx} className="hover:bg-gray-50/50">
                ...
                <td className="py-3 px-3">
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-gray-500 mt-0.5 font-mono">
                ...
                <td className="py-3 px-3 text-right font-mono">{formatCurrency(item.unit_price, config)}</td>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-8 mt-6 pt-4 border-t border-gray-200">
        ...
        <div className="flex-1 space-y-4 text-xs max-w-md">
          ...
          <div className="flex items-center gap-4 pt-2">
            ...
            />
            <div>
              <p className="font-mono text-xs font-bold text-gray-900">{transaction.invoice_no}</p>
        ...
        <div className="w-full sm:w-80 bg-gray-50 border border-gray-200 rounded-xl p-4 text-xs space-y-2">
          <div className="flex justify-between text-gray-600">
            <span>Subtotal Amount:</span>
            <span className="font-mono font-semibold">{formatCurrency(totals.subtotal, config)}</span>
          </div>
          {totals.discount_total > 0 && (
            <div className="flex justify-between text-green-700">
              <span>Total Discount:</span>
              <span className="font-mono font-semibold">-{formatCurrency(totals.discount_total, config)}</span>
            </div>
          )}
          {totals.tax_total > 0 && (
            <div className="flex justify-between text-gray-600">
              <span>{config.tax_label || "VAT"}:</span>
              <span className="font-mono font-semibold">{formatCurrency(totals.tax_total, config)}</span>
            </div>
          )}
          ...
          <div className="pt-2 border-t border-gray-200 space-y-1 text-xs">
            <div className="flex justify-between text-gray-700">
              ...
              <span className="font-mono">{formatCurrency(totals.paid_amount, config)}</span>
            </div>
            {totals.change_amount > 0 && (
      ...
      {/* Signature Row */}
      <div className="hidden print:flex justify-between items-end pt-16 mt-8 border-t border-gray-200 text-xs text-gray-600">
        <div className="text-center">
          <div className="w-48 border-t border-gray-400 mb-1" />
