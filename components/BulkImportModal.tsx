...
export default function BulkImportModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <div className="bg-gray-950 border border-white/10 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-white">Bulk Product Import</h2>
              <p className="text-xs text-gray-400">Import products from CSV with automatic barcode generation</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          ...
          {step === "upload" && (
            <div className="py-8 flex flex-col items-center justify-center text-center space-y-4">
              <div
                ...
                className="w-full max-w-lg border-2 border-dashed border-gray-700 hover:border-green-500/60 rounded-3xl p-8 sm:p-12 cursor-pointer transition-all bg-white/5 hover:bg-white/10 flex flex-col items-center justify-center group"
              >
                <div className="w-16 h-16 rounded-2xl bg-green-500/20 text-green-400 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <FileText size={32} />
                </div>
                <h3 className="text-base sm:text-lg font-bold text-white">Click or drag & drop CSV file</h3>
                <p className="text-xs text-gray-400 mt-1">Supports UTF-8 CSV, Comma/Semicolon/Tab separated files</p>
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              <div className="text-xs text-gray-500 max-w-md">
                💡 <span className="text-gray-400">Tip:</span> If the Barcode column is empty, Autopilot POS will
          ...
          {step === "map" && (
            <div className="space-y-5">
              <div className="bg-white/5 p-4 rounded-xl border border-white/10 text-xs text-gray-300 flex items-center justify-between">
                <span>
                  Found <strong className="text-white">{rawRows.length} rows</strong> in CSV file.
                </span>
                <span className="text-green-400 font-semibold">Match CSV headers to database fields</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 rounded-xl bg-black border border-gray-700 text-white text-xs"
                  >
                ...
                <div>
                  <label className="text-xs font-bold text-gray-300 block mb-1">SKU / Item Code</label>
                  <select
                    value={mapping.sku || ""}
                ...
                <div>
                  <label className="text-xs font-bold text-gray-300 block mb-1">Cost / Buy Price</label>
                  <select
                ...
                <div>
                  <label className="text-xs font-bold text-gray-300 block mb-1">Initial Stock Quantity</label>
                  <select
                    value={mapping.stock || ""}
                ...
                <div>
                  <label className="text-xs font-bold text-gray-300 block mb-1">Min / Alert Stock</label>
                  <select
          ...
          {step === "preview" && (
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs bg-white/5 p-3.5 rounded-xl border border-white/10">
                <div className="flex items-center gap-3">
                ...
                <span className="text-gray-400">Total {validatedRows.length} items ready for processing</span>
              </div>

              <div className="max-h-72 overflow-x-auto overflow-y-auto border border-white/10 rounded-xl bg-black/40">
                <table className="w-full text-left text-xs text-gray-300">
                  <thead className="bg-white/10 text-white text-[11px] sticky top-0">
                    <tr>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {validatedRows.slice(0, 50).map((r, i) => (
                      <tr key={i} className={r.errors ? "bg-red-500/10" : "hover:bg-white/5"}>
                        <td className="p-2.5">
                          {r.errors ? (
                            <span className="text-red-400 font-bold" title={r.errors.join(", ")}>
                              ⚠️ Error
                            </span>
                          ) : (
                        ...
                        <td className="p-2.5 font-mono text-[11px]">
                          {r.barcode} {r.is_generated_barcode && <span className="text-green-400 text-[9px]">(Auto)</span>}
                        </td>
                        <td className="p-2.5 text-gray-400">{r.category}</td>
                        <td className="p-2.5">{r.buy_price}</td>
                        <td className="p-2.5 font-bold text-white">{r.sell_price}</td>
                        <td className="p-2.5 font-semibold text-green-400">{r.stock}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
              {importing ? (
                <>
                  ...
                  <h3 className="text-base font-bold text-white">Importing Products to Catalog...</h3>
                  <p className="text-xs text-gray-400">Creating store products and recording inventory ledger entries</p>
                </>
              ) : importStats ? (
                <>
                  ...
                  <div className="flex gap-4 text-sm font-semibold">
                    <span className="text-green-400">{importStats.imported} Products Created</span>
                  ...
                  <button
                    ...
                    className="mt-4 px-6 py-2.5 rounded-xl bg-green-500 hover:bg-green-600 text-black font-bold text-xs"
                  >
                    Done
                  </button>
                </>
        ...
        {step !== "importing" && (
          <div className="p-4 sm:p-6 border-t border-white/10 flex items-center justify-between gap-3 shrink-0">
            {step === "map" && (
              <button
                type="button"
            ...
            <div className="ml-auto flex items-center gap-3">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 text-xs font-semibold"
              >
                Cancel
              </button>
              ...
              {step === "map" && (
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-green-500 hover:bg-green-600 text-black text-xs font-bold flex items-center gap-1.5 shadow-lg active:scale-95"
                >
